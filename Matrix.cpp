...
Matrix33::Matrix33(double m11, double m12, double m13, double m21, double m22, double m23, double m31, double m32,
                   double m33) {
    m_matrix[0][0] = m11;
    m_matrix[0][1] = m12;
    m_matrix[0][2] = m13;
...
Matrix33 Matrix33::operator+( Matrix33& other) const {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            result.get(i,j) = m_matrix[i][j] + other.get(i,j);
        }
    }
    return result;
}
...
Matrix33 Matrix33::operator*(Matrix33 &other) const {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            for (int k = 0; k < 3; k++) {
                result.get(i,j) += m_matrix[i][k] * other.get(k,j);
            }
        }
    }
    return result;
...
Matrix33 Matrix33::operator*(int scalar) const {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            result.get(i,j) = m_matrix[i][j] * scalar;
        }
    }
    return result;
}
